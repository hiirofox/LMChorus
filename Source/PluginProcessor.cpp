...
void LModelAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	float taps = *Params.getRawParameterValue("taps");
	float delay = *Params.getRawParameterValue("delay");
	float depth = *Params.getRawParameterValue("depth");
	float rate = *Params.getRawParameterValue("rate");
	float spread = *Params.getRawParameterValue("spread");
	float mix = *Params.getRawParameterValue("mix");

	chorus.SetParameters((int)taps, delay, depth, rate, spread, mix);
	chorus.ProcessBlock(recbufl, recbufr, wavbufl, wavbufr, numSamples);
}
